...
class Comment extends Model{
    ...
    public function addComment($data)
    {
        ...
        if ($category_id != 7) {
            ...
            $sql = "INSERT INTO comments (title, content, user_id, news_id, category_id, status)
    ...
    public function getCommentListByNews($news_id, $page)
    {
        ...
        $sql = "SELECT c.id AS comment_id, c.title, c.content, c.likes, c.dislikes, c.status, a.id
                FROM comments AS c
                JOIN articles AS a ON c.news_id = a.id
                WHERE a.id = '{$news_id}'
                ORDER BY `date` DESC
